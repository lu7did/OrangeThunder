...
int main(int argc, char* argv[])
{
	...
	iqtest.SetPLLMasterLoop(3,4,0);
        ...
        fprintf(stderr,"bogus: starting operation with mode(%d)\n",iqtest.ModeIQ);
	...
	while(running)
	{
			...
			switch(InputType)
			{
				...
				break;
				case typeiq_float:   //// ESTE ES EL QUE HAY QUE CAMBIAR
				{
					...
					if(nbread>0)
					{
		 		         for(int i=0;i<nbread/2;i++)
					 {
					   if(i%Decimation==0)
					   {
                                             if (IQBuffer[i*2]==1111.0 && IQBuffer[i*2+1]==1111.0){
                                                IQBuffer[i*2]=0.0;
                                                IQBuffer[i*2+1]=0.0;
                                                iqtest.ModeIQ=MODE_IQ;
                                                //fprintf(stderr,"bogus: change from MODE_FREQ_A --> MODE_IQ\n");
                                             } 
                                             if (IQBuffer[i*2]==2222.0 && IQBuffer[i*2+1]==2222.0){
                                                IQBuffer[i*2]=0.0;
                                                IQBuffer[i*2+1]=0.0;
                                                iqtest.ModeIQ=MODE_FREQ_A;
                                                //fprintf(stderr,"bogus: change from MODE_IQ --> MODE_FREQ_A\n");
                                             } 

 					     if (iqtest.ModeIQ==MODE_FREQ_A) { 	
                                                 IQBuffer[i*2]=100.0;
                                                 IQBuffer[i*2+1]=10.0;
                                             }
