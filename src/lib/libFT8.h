...
float signalDSP[256000];        //only 12000 used
...
const float fft_norm = 2.0f / nfft;
    ...
    (TRACE>=0x02 ? fprintf(stderr,"%s:extract_power Block size=%d Subblock size=%d N_FFT=%d FFT work area=%lu\n",PROGRAMID,block_size,subblock_size,nfft,fft_work_size) : _NOP);
   ...
   //(TRACE>=0x00 ? fprintf(stderr,"%s:extract power Max magnitude: %.1f dB\n",max_mag) : _NOP);
   free(fft_work);
}
...
void FT8_process(int num_samples,int sample_rate) {
    ...
    (TRACE >=0x03 ? fprintf(stderr,"%s:FT8_process returned from normalize_signal\n",PROGRAMID) : _NOP);
    ...
    (TRACE >=0x03 ? fprintf(stderr,"%s:FT8_process sample rate %d Hz, %d blocks, %d bins\n",PROGRAMID,sample_rate,num_blocks,num_bins) : _NOP);
    ...
    (TRACE >=0x03 ? fprintf(stderr,"%s:FT8_process expract_power processed\n",PROGRAMID) : _NOP);
    ...
    (TRACE >=0x03 ? fprintf(stderr,"%s:FT8_process find top candidates candidates(%d)\n",PROGRAMID,num_candidates) : _NOP);
    ...
    for (int idx = 0; idx < num_candidates; ++idx) {
        ...
        if (n_errors > 0) {
            ...
            (TRACE >=0x03 ? fprintf(stderr,"%s:FT8_process ldpc_decode() = %d (%.0f Hz Hz)\n",PROGRAMID,n_errors,freq_hz) : _NOP);
        ...
        (TRACE >=0x03 ? fprintf(stderr,"%s:FT8_process about to extract payload\n",PROGRAMID) : _NOP);
        ...
        (TRACE >=0x03 ? fprintf(stderr,"%s:FT8_process checked for duplicates decoded(%d)\n",PROGRAMID,num_decoded) : _NOP);
        ...
        if (!found && num_decoded < kMax_decoded_messages) {
            ...
            fprintf(stderr,"(%3d %4.1f %4d ~  %s)\n", cand.score, time_sec, (int)(freq_hz + 0.5f), message);
        }
    }
    //LOG(LOG_INFO, "Decoded %d messages\n", num_decoded);
    (TRACE >=0x03 ? fprintf(stderr,"%s:FT8_process decoded %d messages\n",PROGRAMID,num_decoded) : _NOP);
