...
int gpioSIGTERM() {
    ...
    (TRACE >=0x00 ? fprintf(stderr, "\n%s:gpioSignalHandler() SIGTERM caught, exiting!\n",PROGRAMID) : _NOP);
    return 0;
...
int  k=0;
   ...
   if (level == 0) {
      ...
      if (lapPush < MINSWPUSH) {
         ...
         //gpio[g].BMULTI=true;
         if (mtime > MAXSWPUSH) {
            (TRACE >= 0x02 ? fprintf(stderr,"%s:handleGPIOAlert() Pin(%d) pushed long t(%ld) ms.\n",PROGRAMID,g,mtime) : _NOP);
            gpio[g].KDOWN=true;
         } else {
...
void updateEncoders(int gpio, int level, uint32_t tick)
{
        ...
        clkState=gpioRead(GPIO_CLK);
        dtState= gpioRead(GPIO_DT);

        endEncoder = std::chrono::system_clock::now();
        lapEncoder=std::chrono::duration_cast<std::chrono::milliseconds>(endEncoder - startEncoder).count();

        if ( lapEncoder  < MINENCLAP )  {
             (TRACE>=0x02 ? fprintf(stderr,"%s:updateEncoders: ignore pulse too close from last\n",PROGRAMID) : _NOP);
             return;
        }
...
// *--------------------------------------------------------------------------------
        ...
        while ( 1 ) {
             int opt = getopt(argc, argv, "p:v:ehq");
             if(opt == -1) {
   	       if(anyargs) break;
...
// *--------------------------------------------------------------------------------
   ...
   gpioSetSignalFunc(SIGTERM,(gpioSignalFunc_t)gpioSIGTERM);
   gpioSetSignalFunc(SIGINT,(gpioSignalFunc_t)gpioSIGTERM);
   ...
   if (ngpio!=0 || encoder==true) {
      (TRACE >= 0x02 ? fprintf(stderr,"%s:main() %d gpio pin or encoder(%s) informed and activated.\n",PROGRAMID,ngpio,BOOL2CHAR(encoder)) : _NOP);
   } else {
